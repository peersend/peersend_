...
// Copyright (c) 2018-2019 The PeerSend developers
...
#if defined(NDEBUG)
#error "peersend cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uPSD) are considered zero fee (for relaying and mining)
...
bool ContextualCheckZerocoinSpend(const CTransaction& tx, const CoinSpend& spend, CBlockIndex* pindex, const uint256& hashBlock)
{
    ...
    if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
        return error("%s : zpsd spend with serial %s is already in block %d\n", __func__,
...
{
    //Check to see if the zpsd is properly signed
    if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
        try {
            if (!spend.HasValidSignature())
                return error("%s: V2 zpsd spend does not have a valid signature\n", __func__);
        ...
        if (spend.getSpendType() != expectedType) {
            return error("%s: trying to spend zpsd without the correct spend type. txid=%s\n", __func__,
                         tx.GetHash().GetHex());
        }
    }
    ...
    if (!spend.HasValidSerial(Params().Zerocoin_Params(fUseV1Params))) {
        ...
        if (!isBlockBetweenFakeSerialAttackRange(pindex->nHeight))
            return error("%s : zpsd spend with serial %s from tx %s is not in valid range\n", __func__,
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zpsd spend tx %s already in block %d",
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
                    return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                               tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zpsd");
            }
        } else {
            LOCK(pool.cs);
            ...
            // Check that zpsd mints are not already known
...
int64_t GetBlockValue(int nHeight)
{
	...
	nSubsidy = 0 * COIN;
        ...
        nSubsidy = 10 * COIN;
	} else if (nHeight <= 230000 && nHeight > Params().LAST_POW_BLOCK()) { //Start PoS
        nSubsidy = 10 * COIN;
	} else if (nHeight <= 750000 && nHeight >= 230001) {
        nSubsidy = 7.5 * COIN;
	} else if (nHeight <= 1500000 && nHeight >= 750001) {
        nSubsidy = 5 * COIN;
	}    else {
        nSubsidy = 5 * COIN;
	}
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool iszpsdStake){
    int64_t ret = 0;
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from peersend
...
{
    RenameThread("peersend-scriptch");
...
void RecalculatezpsdMinted()
{
    CBlockIndex *pindex = chainActive[Params().Zerocoin_StartHeight()];
    uiInterface.ShowProgress(_("Recalculating minted zpsd..."), 0);
    while (true) {
        ...
        if (pindex->nHeight % 1000 == 0) {
            ...
            uiInterface.ShowProgress(_("Recalculating minted zpsd..."), percent);
        }
...
void RecalculatezpsdSpent()
{
    CBlockIndex* pindex = chainActive[Params().Zerocoin_StartHeight()];
    uiInterface.ShowProgress(_("Recalculating spent zpsd..."), 0);
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
            int percent = std::max(1, std::min(99, (int)((double)(pindex->nHeight - Params().Zerocoin_StartHeight()) * 100 / (chainActive.Height() - Params().Zerocoin_StartHeight()))));
            uiInterface.ShowProgress(_("Recalculating spent zpsd..."), percent);
        }
        ...
        //Remove spends from zpsd supply
...
bool RecalculatePSDSupply(int nHeightStart)
{
    ...
    uiInterface.ShowProgress(_("Recalculating PSD supply..."), 0);
    while (true) {
        if (pindex->nHeight % 1000 == 0) {
            LogPrintf("%s : block %d...\n", __func__, pindex->nHeight);
            int percent = std::max(1, std::min(99, (int)((double)((pindex->nHeight - nHeightStart) * 100) / (chainActive.Height() - nHeightStart))));
            uiInterface.ShowProgress(_("Recalculating PSD supply..."), percent);
        }
        ...
        if(pindex->nHeight == 2195){
            //500k PSD Burnt in tx f9c851fddc613db8ce3469b88faa28c373ed7881a752d4aac28a0463cd42db8e
            pindex->nMoneySupply -= 500000 * COIN;

        }
        if(pindex->nHeight == 2216){
            //717,064.07 PSD Burnt in tx 243e04d31933a998462b4f13f013cfa97e994a8d82141c7a9ce1d6ff45d58588
...
{
    // peersend: recalculate Accumulator Checkpoints that failed to database properly
...
bool UpdatezpsdSupply(const CBlock& block, CBlockIndex* pindex, bool fJustCheck)
{
    std::list<CZerocoinMint> listMints;
    ...
    // A one-time event where only the zpsd supply was off (due to serial duplication off-chain on main net)
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
        RecalculatezpsdMinted();
        RecalculatezpsdSpent();
        RecalculatePSDSupply(Params().Zerocoin_StartHeight());
    }

    //Track zpsd money supply in the block index
    if (!UpdatezpsdSupply(block, pindex, fJustCheck))
        return state.DoS(100, error("%s: Failed to calculate new zpsd supply for block=%s height=%d", __func__,
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zpsdSpent: %s\n",
...
/*
#ifdef ENABLE_WALLET
    // If turned on AutoZeromint will automatically convert PSD to zpsd
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zpsd spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn& txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zpsd serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    if (nHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s: zpsd stake block is less than allowed start height", __func__);

    if (CzpsdStake* zpsd = dynamic_cast<CzpsdStake*>(stake)) {
        CBlockIndex* pindexFrom = zpsd->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with zpsd stake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zpsd stake does not have required confirmation depth. Current height %d,  stakeInput height %d.", __func__, chainActive.Height(), pindexFrom->nHeight);
        ...
        uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zpsd->GetValue()));
        if (nChecksum200 != zpsd->GetChecksum())
            return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zpsd->GetChecksum(), nChecksum200);
    } else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (stake->Iszpsd() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
            return state.DoS(100, error("%s: staked zpsd fails context checks", __func__));
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> psdInputs;
        std::vector<CTxIn> zpsdInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
            if(stakeIn.scriptSig.IsZerocoinSpend()){
                zpsdInputs.push_back(stakeIn);
            }else{
                psdInputs.push_back(stakeIn);
            }
        }
        const bool hasPSDInputs = !psdInputs.empty();
        const bool haszpsdInputs = !zpsdInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasPSDInputs)
                    ...
                    for (const CTxIn& PSDIn : psdInputs){
                        if(PSDIn.prevout == in.prevout){
        ...
        if (isBlockFromFork) {
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const CTransaction &t : bl.vtx) {
                    for (const CTxIn &in: t.vin) {
                        ...
                        for (const CTxIn &stakeIn : psdInputs) {
            ...
            // Now that this loop if completed. Check if we have zpsd inputs.
            if(haszpsdInputs){
                for (const CTxIn& zpsdInput : zpsdInputs) {
                    CoinSpend spend = TxInToZerocoinSpend(zpsdInput);
                    ...
                    if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, spend, pindex, 0))
                        return state.DoS(100,error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                                   stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zpsd");
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zpsd mints and %d zpsd spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // peersend: We use certain sporks during IBD, so check to see if they are
      ...
      // cleanSubVer has /PeerSend.../ for us
        ...
        if(nLocalServices & NODE_BLOOM_LIGHT_ZC) {
            try {
                ...
                if (!GetAccumulatorValue(height, den, bnAccValue)) {
                    LogPrint("zpsd", "peer misbehaving for request an invalid acc checkpoint \n", __func__);
                    Misbehaving(pfrom->GetId(), 50);
                } else {
            ...
            try {
                ...
                if (gen.isValid(chainActive.Height())) {
                    if (!lightWorker.addWitWork(gen)) {
                        LogPrint("zpsd", "%s : add genwit request failed \n", __func__);
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
