...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake)
{
    ...
    if (tx.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CzpsdStake(spend));
    } else {
        ...
        CPSDStake* PSDInput = new CPSDStake();
        PSDInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(PSDInput);
    }
