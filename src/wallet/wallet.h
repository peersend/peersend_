...
extern bool fGlobalUnlockSpendCache; // Bool used for letting the precomputing thread know that zpsdspends need to use the cs_spendcache
...
enum AvailableCoinsType {
    ...
    ONLY_DENOMINATED = 2,
    ONLY_NOT10000IFMN = 3,
    ONLY_NONDENOMINATED_NOT10000IFMN = 4, // ONLY_NONDENOMINATED and not 10000 PSD at the same time
    ONLY_10000 = 5,                        // find masternode outputs including locked ones (use with caution)
    STAKABLE_COINS = 6                          // UTXO's that are valid for staking
};

// Possible states for zpsd send
enum ZerocoinSpendStatus {
    zpsd_SPEND_OKAY = 0,                            // No error
    zpsd_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    zpsd_WALLET_LOCKED = 2,                         // Wallet was locked
    zpsd_COMMIT_FAILED = 3,                         // Commit failed, reset status
    zpsd_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    zpsd_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    zpsd_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    zpsd_TRX_CREATE = 7,                            // Everything related to create the transaction
    zpsd_TRX_CHANGE = 8,                            // Everything related to transaction change
    zpsd_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    zpsd_INVALID_COIN = 10,                         // Selected mint coin is not valid
    zpsd_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    zpsd_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    zpsd_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    zpsd_SPENT_USED_zpsd = 14,                      // Coin has already been spend
    zpsd_TX_TOO_LARGE = 15,                          // The transaction is larger than the max tx size
    zpsd_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    void zpsdBackupWallet();
    bool GetZerocoinKey(const CBigNum& bnSerial, CKey& key);
    bool CreatezpsdOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzpsdAuto) const;
    ...
    */
    ...
    bool fBackupMints;
    std::unique_ptr<CzpsdTracker> zpsdTracker;
    ...
    void setZWallet(CzpsdWallet* zwallet)
    {
        zwalletMain = zwallet;
        zpsdTracker = std::unique_ptr<CzpsdTracker>(new CzpsdTracker(strWalletFile));
    }

    CzpsdWallet* getZWallet() { return zwalletMain; }
    ...
    void setzpsdAutoBackups(bool fEnabled)
    {
    ...
    /// Get 5000PSD input and keys which can be used for the Masternode
    ...
    /** zpsd reset */
    boost::signals2::signal<void()> NotifyzpsdReset;
