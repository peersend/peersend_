...
CzpsdWallet::CzpsdWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zpsd seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadzpsdSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.ErasezpsdSeed_deprecated()) {
                    LogPrintf("%s: Updated zpsd seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zpsd seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zpsd wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzpsdWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WritezpsdCount(nCountLastUsed);
    else if (!walletdb.ReadzpsdCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzpsdWallet::Lock()
{
    seedMaster = 0;
}

void CzpsdWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzpsdWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedTozpsd(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzpsdWallet::LoadMintPoolFromDB()
{
...
void CzpsdWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzpsdWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzpsdWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zpsdTracker->HasPubcoinHash(pMint.first)) {
...
bool CzpsdWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedTozpsd(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zpsdTracker which also adds to database
    pwalletMain->zpsdTracker->Add(dMint, true);
...
void CzpsdWallet::SeedTozpsd(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzpsdWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzpsdWallet::GenerateDeterministiczpsd(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzpsdWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzpsdWallet::CheckSeed(const CDeterministicMint& dMint)
{
...
bool CzpsdWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
    if (!CheckSeed(dMint)) {
        uint256 hashSeed = Hash(seedMaster.begin(), seedMaster.end());
