// Copyright (c) 2018 The peersend developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include <zpsd/deterministicmint.h>
#include "zpsdtracker.h"
#include "util.h"
#include "sync.h"
#include "main.h"
#include "txdb.h"
...
CzpsdTracker::CzpsdTracker(std::string strWalletFile)
{
...
CzpsdTracker::~CzpsdTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzpsdTracker::Init()
{
...
bool CzpsdTracker::Archive(CMintMeta& meta)
{
...
bool CzpsdTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzpsdTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzpsdTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzpsdTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
CoinWitnessData* CzpsdTracker::GetSpendCache(const uint256& hashStake)
{
...
bool CzpsdTracker::ClearSpendCache()
{
    AssertLockHeld(cs_spendcache);
...
std::vector<uint256> CzpsdTracker::GetSerialHashes()
{
...
CAmount CzpsdTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzpsdTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzpsdTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzpsdTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzpsdTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzpsdTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzpsdTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzpsdTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzpsdTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzpsdTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzpsdTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived, CzpsdWallet* zpsdWallet)
{
    bool iszpsdWalletInitialized = (NULL != zpsdWallet);
    CMintMeta meta;
    ...
    if (! iszpsdWalletInitialized)
        zpsdWallet = new CzpsdWallet(strWalletFile);
    meta.isSeedCorrect = zpsdWallet->CheckSeed(dMint);
    if (! iszpsdWalletInitialized)
        delete zpsdWallet;
    mapSerialHashes[meta.hashSerial] = meta;
...
void CzpsdTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzpsdTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzpsdTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzpsdTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzpsdTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzpsdTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus, bool fWrongSeed)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        CzpsdWallet* zpsdWallet = new CzpsdWallet(strWalletFile);
        for (auto& dMint : listDeterministicDB) {
            Add(dMint, false, false, zpsdWallet);
        }
        delete zpsdWallet;
        LogPrint("zero", "%s: added %d dzpsd from DB\n", __func__, listDeterministicDB.size());
    }
