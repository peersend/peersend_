...
#include "ui_zpsdcontroldialog.h"
...
std::set<std::string> zpsdControlDialog::setSelectedMints;
std::set<CMintMeta> zpsdControlDialog::setMints;

bool CzpsdControlWidgetItem::operator<(const QTreeWidgetItem &other) const {
    int column = treeWidget()->sortColumn();
    if (column == zpsdControlDialog::COLUMN_DENOMINATION || column == zpsdControlDialog::COLUMN_VERSION || column == zpsdControlDialog::COLUMN_CONFIRMATIONS)
        return data(column, Qt::UserRole).toLongLong() < other.data(column, Qt::UserRole).toLongLong();
    return QTreeWidgetItem::operator<(other);
}


zpsdControlDialog::zpsdControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::zpsdControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
zpsdControlDialog::~zpsdControlDialog()
{
    delete ui;
}

void zpsdControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void zpsdControlDialog::updateList()
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        CzpsdControlWidgetItem* itemDenom(new CzpsdControlWidgetItem);
    ...
    for (const CMintMeta& mint : setMints) {
        ...
        CzpsdControlWidgetItem *itemMint = new CzpsdControlWidgetItem(ui->treeWidget->topLevelItem(mapDenomPosition.at(denom)));
        ...
        {
            LOCK(pwalletMain->zpsdTracker->cs_spendcache);

            CoinWitnessData *witnessData = pwalletMain->zpsdTracker->GetSpendCache(mint.hashStake);
        ...
        if(!fSpendable) {
            ...
            if(nConfirmations < Params().Zerocoin_MintRequiredConfirmations())
                ...
                strReason = "Your wallet is locked. Impossible to precompute or spend zpsd.";
            else if (!mint.isSeedCorrect)
                strReason = "The zpsd seed used to mint this zpsd is not the same as currently hold in the wallet";
            else
...
void zpsdControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void zpsdControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelzpsd_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setzpsdControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> zpsdControlDialog::GetSelectedMints()
{
...
void zpsdControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
