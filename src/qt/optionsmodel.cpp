...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::PSD);
    ...
    if (!settings.contains("nAnonymizepeersendAmount"))
        settings.setValue("nAnonymizepeersendAmount", 1000);

    nAnonymizepeersendAmount = settings.value("nAnonymizepeersendAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizepeersendAmount"))
        SoftSetArg("-anonymizepeersendamount", settings.value("nAnonymizepeersendAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in peersend.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizepeersendAmount:
            return QVariant(nAnonymizepeersendAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case HideOrphans:
            ...
            break;
        case AnonymizepeersendAmount:
            nAnonymizepeersendAmount = value.toInt();
            settings.setValue("nAnonymizepeersendAmount", nAnonymizepeersendAmount);
            emit anonymizepeersendAmountChanged(nAnonymizepeersendAmount);
            break;
        case CoinControlFeatures:
