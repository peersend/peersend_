...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.IsZerocoinSpend() && (fZSpendFromMe || wallet->zpsdTracker->HasMintTx(hash))) {
            //zpsd stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::Stakezpsd;
        ...
        for (const CTxOut& txout : wtx.vout) {
            ...
            if (txout.IsZerocoinMint()) {
                ...
                TransactionRecord sub(hash, nTime);
                sub.involvesWatchAddress = mine & ISMINE_WATCH_ONLY;
                sub.type = TransactionRecord::ZerocoinSpend_Change_zpsd;
                sub.address = mapValue["zerocoinmint"];
                if (!fFeeAssigned) {
...
bool IszpsdType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::Stakezpsd:
        case TransactionRecord::ZerocoinMint:
        case TransactionRecord::ZerocoinSpend:
        case TransactionRecord::RecvFromZerocoinSpend:
        case TransactionRecord::ZerocoinSpend_Change_zpsd:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::Stakezpsd || type == TransactionRecord::MNReward) {
