                                                          ...
                                                          fDenomsMinimized(true)
{
    ...
    // "Spending 999999 zpsd ought to be enough for anybody." - Bill Gates, 2017
    ui->zpsdpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzpsdSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zpsdpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzpsd_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zpsd is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_zpsd, true));
        if (!ctx.isValid()) {
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zpsd...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zpsd in ") +
...
void PrivacyDialog::on_pushButtonSpendzpsd_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zpsd is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_zpsd, true));
        if (!ctx.isValid()) {
            // Unlock wallet was cancelled
            return;
        }
        // Wallet is unlocked now, sedn zpsd
        sendzpsd();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zpsd
    sendzpsd();
}

void PrivacyDialog::on_pushButtonzpsdControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    zpsdControlDialog* zpsdControl = new zpsdControlDialog(this);
    zpsdControl->setModel(walletModel);
    zpsdControl->exec();
}

void PrivacyDialog::setzpsdControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzpsdSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzpsd()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid peersend Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zpsdpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zpsd is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " PSD </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zpsd</b>";
    ...
    // use mints from zpsd selector if applicable
    vector<CMintMeta> vMintsToFetch;
    vector<CZerocoinMint> vMintsSelected;
    if (!zpsdControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = zpsdControlDialog::GetSelectedMints();
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zpsd was spent successfully update the addressbook with the label
    ...
    // Clear zpsd selector in case it was used
    zpsdControlDialog::setSelectedMints.clear();
    ui->labelzpsdSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zpsd Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " PSD, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zpsd Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    std::vector<CMintMeta> vMints = pwalletMain->zpsdTracker->GetMints(false);
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zpsd </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zpsd "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zpsd "));
    ui->labelzAvailableAmount_4->setText(QString::number(zerocoinBalance/COIN) + QString(" zpsd "));
    ui->labelzpsdAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zpsd </b> "));
    ui->labelZsupplyAmount_2->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zpsd </b> "));
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zpsd </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzpsdSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    bool fButtonsEnabled =  ui->pushButtonMintzpsd->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zpsd
        ui->pushButtonMintzpsd->setEnabled(false);
        ui->pushButtonMintzpsd->setToolTip(tr("zpsd is currently disabled due to maintenance."));

        // Spend zpsd
        ui->pushButtonSpendzpsd->setEnabled(false);
        ui->pushButtonSpendzpsd->setToolTip(tr("zpsd is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zpsd
        ui->pushButtonMintzpsd->setEnabled(true);
        ui->pushButtonMintzpsd->setToolTip(tr("PrivacyDialog", "Enter an amount of PSD to convert to zpsd", 0));

        // Spend zpsd
        ui->pushButtonSpendzpsd->setEnabled(true);
        ui->pushButtonSpendzpsd->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}
